...
Cookie は 4KB が上限で、取込 1 行は UUID を 7 個持つ。素の deflate では 2 倍程度にしか
縮まないため、**2 回以上現れる文字列を辞書へ移してから** deflate している
（`src/lib/repositories/demo-edit-codec.ts`）。実測でおよそ 10 倍になり、容量は
**取込 5 行程度から 45 行程度へ**広がった。

**残る制約（本番で実測）**: 本番 Demo Mode の一括取込は **2 ファイル / 約 25 行**までなら
プレビューまで到達する。3 ファイル以上（それを超える行数）は Cookie に収まらず、
インスタンスが変わると参照できない。その場合は取込直後に限り、理由と回避策を示す画面を出す
（存在秘匿のため、それ以外の未知 ID は 404 のまま）。

| 一括取込                  | 本番 Demo Mode                             | ローカル Demo Mode | Supabase Mode |
| ------------------------- | ------------------------------------------ | ------------------ | ------------- |
| 2 ファイル / 約 25 行まで | 動く                                       | 動く               | 動く          |
| 3 ファイル以上            | インスタンスが変わると参照不可（説明画面） | 動く               | 動く          |
| 50 ファイル一括           | 同上                                       | 動く               | 動く          |

要求仕様の「事前加工なしで 50 ファイルを一括取込」は、ローカル Demo Mode と Supabase Mode では
満たしている。本番だけ制限されるのは、**環境変数を一切設定しない**という運用方針により
サーバー側の保存先を持てないためで、アプリの実装上の欠落ではない。
...
| #   | 制約                                                                                                                              |
| --- | --------------------------------------------------------------------------------------------------------------------------------- |
| M-1 | CDP / SSBJ は**架空の縮小マスター**（CDP 12 問 / SSBJ 10 項目）。正式質問書ではない。画面に「架空の縮小マスター」バッジを常時表示 |
